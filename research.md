...
1. **Evaluator** (`evaluator.ts`) — single LLM call: pure scope gate. Validates BC aviation scope only. Out-of-scope → helpful rejection. Returns `{ status, reason }` — no section routing, no aerodrome extraction.
...
5. **Vision escalation** (`visionSearch.ts`) — uses `airportRefs` from the decomposer (or raw question if none) to locate PDF pages. Renders pages as images; Claude reads them for ground truth.
...
| `src/lib/schemas.ts`                   | JSON Schema objects for evaluator, decomposer, synthesizer — passed via `--json-schema` |
...
- **Structured output via `--json-schema`** — `runClaude<T>` in `claudeUtils.ts` passes a JSON Schema to the CLI; the result is read from `structured_output` in the response envelope (not `result`). Schemas live in `schemas.ts`. Anthropic API constraints: top-level must be `type: "object"`, no top-level `oneOf`/`allOf`/`anyOf`; arrays must be wrapped.
