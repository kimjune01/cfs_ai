...
- **Dual query per ICAO** — `rephraseMultipleQueries` generates two queries per aerodrome: `"<name> <topic>"` and `"<ICAO> <topic>"`. Both run in parallel; results deduplicated by start page keeping the higher score. A single `vector_results` trace event is emitted after deduplication (not per query). Improves recall when the aerodrome name is referenced in unrelated chunks (e.g. VOR references).
