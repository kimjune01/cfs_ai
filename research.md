...
| `src/lib/schemas.ts`                   | JSON Schema objects for evaluator, decision, and query rewriter — passed via `--json-schema` |
...
- **Structured output via `--json-schema`** — `runClaude<T>` in `claudeUtils.ts` passes a JSON Schema to the CLI; the result is read from `structured_output` in the response envelope (not `result`). Schemas live in `schemas.ts`. Anthropic API constraints: top-level must be `type: "object"`, no top-level `oneOf`/`allOf`/`anyOf`; arrays must be wrapped (e.g. `{ queries: string[] }`).
...
- **Dual query per ICAO** — `rephraseMultipleQueries` generates two queries per aerodrome: `"<name> <topic>"` and `"<ICAO> <topic>"`. Both run in parallel; results deduplicated by chunk text identity (keeping the higher score if the same chunk appears in both result sets). A single `vector_results` trace event is emitted after deduplication (not per query). Improves recall when the aerodrome name is referenced in unrelated chunks (e.g. VOR references).
